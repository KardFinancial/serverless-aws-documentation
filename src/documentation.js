...
module.exports = function() {
	return {
		_createDocumentationPart: function _createDocumentationPart(
			part,
			def,
			knownLocation
		) {
		...
		createDocumentationPart: function createDocumentationPart(
			part,
			def,
			knownLocation
		) {
		...
		createDocumentationParts: function createDocumentationParts(
			parts,
			def,
			knownLocation
		) {
		...
		_updateDocumentation: function _updateDocumentation() {
			...
			return aws
				.request('APIGateway', 'getDocumentationVersion', {
					restApiId: this.restApiId,
				...
				.then(() =>
					aws.request('APIGateway', 'createDocumentationVersion', {
						restApiId: this.restApiId,
						documentationVersion: this.getDocumentationVersion(),
		...
		getFunctionDocumentationParts: function getFunctionDocumentationParts() {
			...
			Object.keys(httpEvents).forEach(funcNameAndPath => {
				...
				this.createDocumentationParts(functionDocumentationParts, httpEvent, {
					path,
					method
				});
			});
		...
		_getHttpEvents: function _getHttpEvents() {
			return this.serverless.service
				...
				.reduce((documentationObj, functionName) => {
					...
					func.events
						.filter(
							eventTypes => eventTypes.http && eventTypes.http.documentation
						)
						.map(eventTypes => eventTypes.http)
						.forEach(currEvent => {
							if (currEvent.documentation.includeInDocs) {
		...
		generateAutoDocumentationVersion: function generateAutoDocumentationVersion() {
			...
			Object.keys(httpEvents).forEach(funcName => {
				versionObject.functionDocs[funcName] =
					httpEvents[funcName].documentation;
			});
		...
		getDocumentationVersion: function getDocumentationVersion() {
			return (
				this.customVars.documentation.version ||
				autoVersion ||
				this.generateAutoDocumentationVersion()
			);
		},

		_buildDocumentation: function _buildDocumentation(result) {
			this.restApiId = result.Stacks[0].Outputs.filter(
		...
		addDocumentationToApiGateway: function addDocumentationToApiGateway(
			resource,
			documentationPart,
			mapPath
		) {
			if (documentationPart && Object.keys(documentationPart).length > 0) {
				...
				documentationPart.forEach(function(qp) {
					...
					if (resource.Properties.RequestParameters.hasOwnProperty(source))
						return; // don't mess with existing config
		...
		updateCfTemplateFromHttp: function updateCfTemplateFromHttp(eventTypes) {
			if (eventTypes.http && eventTypes.http.documentation) {
				...
				const methodLogicalId = this.getMethodLogicalId(
					resourceName,
					eventTypes.http.method
				);
